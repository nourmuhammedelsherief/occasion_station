    ...
    <div class="page-bar">
        <ul class="page-breadcrumb">
            ...
            @elseif(auth()->guard('admin')->user()->admin_category_id != 4)
                <li>
                    <a href="/employees/home">لوحة التحكم</a>
                    <i class="fa fa-circle"></i>
                </li>
                <li>
                    <a href="/employees/contacts">تواصل معنا</a>
                    <i class="fa fa-circle"></i>
                </li>
            @endif
            <li>
                ...
                <div class="portlet-title">
                    <div class="caption font-dark">
                        ...
                        <span class="caption-subject bold uppercase">
                            @if(auth()->guard('admin')->user()->admin_category_id == 4)
                                <a href="{{route('ArchivedContacts')}}" class="btn btn-success"> المؤرشف </a>
                            @elseif(auth()->guard('admin')->user()->admin_category_id != 4)
                                <a href="{{route('ArchivedContactsE')}}" class="btn btn-success"> المؤرشف </a>
                            @endif
                        </span>
                    </div>
                                ...
                                <td>
                                    ...
                                    @if(auth()->guard('admin')->user()->admin_category_id == 4)
                                        <a class="btn btn-success"
                                           href="{{route('archivedContact' , [$contact->id , 'true'])}}">أرشفة</a>
                                    @elseif(auth()->guard('admin')->user()->admin_category_id != 4)
                                        <a class="btn btn-success"
                                           href="{{route('archivedContactE' , [$contact->id , 'true'])}}">أرشفة</a>
                                    @endif



                                    <a class="delete_attribute btn btn-danger" data="{{$contact->id}}"
                                       data_name="{{$contact->name}}">
