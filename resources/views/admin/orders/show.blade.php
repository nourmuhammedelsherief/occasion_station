...
@section('content')
    ...
    <div class="row">
        <div class="col-sm-6">
            @if(auth()->guard('admin')->user()->admin_category_id == 4 or ($order->status != 'completed' and $order->status != 'canceled'))
                <form action="{{route('ChangeOrderStatus' , $order->id)}}" method="post">
                    @csrf
                    <div class="form-group">
                        <label for="status"> تغيير حاله الطلب </label>
                        <select class="form-control" name="status">
                            <option disabled selected> تغيير حاله الطلب</option>
                            <option value="new_paid" {{$order->status == 'new_paid' ? 'selected' : ''}}>
                                جديد مدفوع
                            </option>
                            <option value="works_on" {{$order->status == 'works_on' ? 'selected' : ''}}>
                                جاري العمل عليه
                            </option>
                            <option
        ...
        <div class="col-sm-6">
            <form action="{{route('Redirection' , $order->id)}}" method="post">
                @csrf
                <div class="form-group">
                    <label for="status"> اعاده توجيه الطلب لموظف أخر </label>
                    <select class="form-control" name="admin_id">
                        <option disabled selected> توجيه الطلب الي</option>
                        @foreach($admins as $admin)
                            <option value="{{$admin->id}}"> {{$admin->name}} </option>
                        @endforeach
                    </select>
                </div>
                <button class="btn btn-primary" type="submit"> توجية</button>
            </form>
        </div>
    </div>
    <hr>
    <div class="row">
        <div class="col-md-12">
            ...
            <div class="portlet light bordered table-responsive">
                <div class="portlet-body">
                    <h3 class="text-center"> ملاحظات الطلب </h3>
                    <div class="table-toolbar">
                        <div class="table-toolbar">
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="btn-group">
                                        <a href="{{route('AdminAddNote' , $order->id)}}">
                                            <button id="sample_editable_1_new"
                                                    class="btn sbold green"> أضافه ملاحظه جديده
