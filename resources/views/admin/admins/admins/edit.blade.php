                ...
                <div class="portlet light bordered">
                    ...
                    <div class="portlet-body form">
                        ...
                        <div class="form-group">
                            <label for="username" class="col-lg-3 control-label">القسم</label>
                            <div class="col-lg-9">
                                <select name="admin_category_id" class="form-control" required>
                                    <option disabled selected> اختر القسم </option>
                                    @foreach($categories as $category)
                                        <option value="{{$category->id}}" {{$data->admin_category_id == $category->id ? 'selected' : ''}}> {{$category->name}} </option>
                                    @endforeach
                                </select>
                                @if ($errors->has('admin_category_id'))
                                    <span class="help-block">
                                       <strong style="color: red;">{{ $errors->first('admin_category_id') }}</strong>
                                    </span>
                                @endif
                            </div>
                        </div>
